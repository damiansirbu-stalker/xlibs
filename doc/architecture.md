...
- **Performance first.** Performance is the top priority and outranks features. A wrapper adds no overhead beyond the bridge call it wraps; one that cannot stay within budget is reworked, or its cost is moved into an engine modification, never carried in script. See `doc/standards/code-standards.md` "Performance is the priority".
- **Use the engine, don't work around it.** xlibs is the release-safe wrapper over the engine's and Anomaly layer's native mechanisms; it never reimplements what they already do. Callers escalate through the wrapper: use it, nudge or correct, and change the layer (an engine modification or a full-file override) only as a last resort. See `doc/standards/code-standards.md` "Use the engine, don't work around it".
